...
// UseInterestingInterfaces reports whether i is an interesting non-Tailscale interface.
...
// UseInterestingIPs reports whether ip is an interesting IP address.
func UseInterestingIPs(ip netaddr.IP) bool {
	return isInterestingIP(ip)
}

// UseAllInterfaces always returns true.
// Use it to make EqualFiltered consider all interfaces.
func UseAllInterfaces(i Interface, ips []netaddr.IPPrefix) bool { return true }

// UseAllIPs returns true for all IPs.
// Use it to make EqualFiltered consider all IPs.
